...
export interface UpstreamTarget { host: string; port: number }
...
const PROXY_IDENTITY_HEADERS = new Set(['forwarded','x-forwarded-for','x-forwarded-host','x-forwarded-proto','cf-connecting-ip','cf-visitor'])
...
export function forwardHeaders(req: IncomingMessage, target: UpstreamTarget, auth: AuthService, keepUpgrade: boolean): Record<string, string | string[] | undefined> {
...
function writeSocketResponse(socket: Duplex, statusCode: number, statusMessage: string, headers: Record<string, string | string[] | undefined>): void {
...
export function proxyHttp(req: IncomingMessage, res: ServerResponse, target: UpstreamTarget, auth: AuthService, logger: Logger): void {
  const upstream = httpRequest({ host: target.host, port: target.port, method: req.method, path: req.url, headers: forwardHeaders(req, target, auth, false) }, (upstreamRes) => {
...
export function proxyUpgrade(req: IncomingMessage, socket: Duplex, head: Buffer, target: UpstreamTarget, auth: AuthService, logger: Logger): void {
  const upstream = httpRequest({ host: target.host, port: target.port, method: req.method, path: req.url, headers: forwardHeaders(req, target, auth, true) })
  upstream.on('upgrade', (upstreamRes: IncomingMessage, upstreamSocket: Duplex, upstreamHead: Buffer) => {
    ...
    for (let i = 0; i + 1 < upstreamRes.rawHeaders.length; i += 2) lines.push(`${upstreamRes.rawHeaders[i]}: ${upstreamRes.rawHeaders[i + 1] ?? ''}`)
    socket.write(`${lines.join('\r\n')}\r\n\r\n`)
  ...
  upstream.on('response', (upstreamRes) => {
    if (socket.destroyed) { upstreamRes.destroy(); return }
    writeSocketResponse(socket, upstreamRes.statusCode ?? 502, upstreamRes.statusMessage ?? 'Bad Gateway', sanitizeResponseHeaders(upstreamRes.headers))
    upstreamRes.pipe(socket, { end: true })
  })
  upstream.on('error', (error) => { logger.warn('token-gate: upstream upgrade error: %s', String(error)); socket.destroy() })

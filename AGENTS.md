...
- The gateway session cookie remains `HttpOnly; Secure; SameSite=Lax`; upstream `Set-Cookie` with the same cookie name must not overwrite it.
...
Run `pnpm run typecheck`, `pnpm run test:coverage`, `pnpm run build`, and `npm pack --dry-run` before release-facing changes. Security-sensitive transport changes should include both allowed and denied/error-path regressions.
